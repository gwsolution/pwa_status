...
export class UtilityServiceTypeClient{
  ...
  public deleteServiceType(id,lang): Observable<Object> {
    return this.http.delete(environment.BASE_URL + environment.UTILITY_SERVICE_TYPE_API+"/"+id+"?lang="+lang, { headers: this.getRequestOptions() });
  }

  public createNewServiceType(serviceType: ServiceType, lang): Observable<Object> {
    console.log(lang)
    return this.http.post(environment.BASE_URL + environment.UTILITY_SERVICE_TYPE_API+"?lang="+lang, serviceType, { headers: this.getRequestOptions() });
  }

  public updateServiceType(serviceType: ServiceType, lang): Observable<Object> {
    return this.http.put(environment.BASE_URL + environment.UTILITY_SERVICE_TYPE_API+"?lang="+lang, serviceType, { headers: this.getRequestOptions() });
  }
}
